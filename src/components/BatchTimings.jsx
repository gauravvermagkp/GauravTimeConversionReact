...
import { updateInputDate, updatecollapseConverted, updateClickedRegion, updateClickedclassName } from '../mySlice.js';
...
export default function BatchTimings({ region }) {
    ...
    const clickedclassName = useSelector((state) => state.timeConverterSlicerName.clickedclassName);
                        ...
                        return <div className={`batch-row batch-${batchname_lower} ${is_running}`}>
                            ...
                            <button className={`btn-soft ${clickedclassName.includes(`${batchname_lower}b1`) ? clickedclassName : ''}`}
                                id={`${batchname_lower}b1`} value={newt[0]} onClick={(e) => {
                                    ...
                                    dispatch(updateClickedclassName(`${e.target.id}-clicked`))
